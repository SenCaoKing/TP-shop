...
class GoodsController extends BackController {
    ...
    public function showlist() {
        ...
        $nowinfo = $goods->fetchData(); //Model里的方法
    ...
    public function tianjia() {
        ...
        /*
          //XSS脚本攻击---收集原生的$_POST表单信息容易形成攻击：
          $goods = new \Model\GoodsModel();
          if (IS_POST) {
          $_POST['add_time'] = time();  //收集原生的表单信息，容易形成攻击(查看表php41_goods第二条数据的goods_introduce字段)
          $_POST['upd_time'] = time();
          if ($goods->add($_POST)) {
          $this->success('添加商品成功', U('showlist'), 2);
          } else {
          $this->error('添加商品失败', U('showlist'), 2);
          }
          } */
        ...
        //俩个逻辑：展示、收集
        if (IS_POST) {//收集表单
            // dump($_POST);exit;  //商品类型/属性信息入库(GoodsModel-_after_insrt)
            ...
            //利用fanXSS函数里边的htmlpurifier对指定的“标签内容”进行过滤
    ...
    public function upd() {
        ...
        if (IS_POST) {
            ...
            /*****获得"商品类型"信息*****/
            $typeinfo = D('Type') -> select(); //修改商品，把类型获得出来传递给模板显示
            $this -> assign('typeinfo',$typeinfo);
            /*****获得"商品类型"信息*****/
    ...
    //删除商品   逻辑方式删除商品，就是修改标志位
    function delGoods() {
        $goods_id = I('get.goods_id'); //获得被删除商品的id信息
        $goods = D('Goods');
        $z = $goods->setField(array('goods_id' => $goods_id, 'is_del' => '删除')); //setField()内部有调用save()方法
        if ($z) {
    ...
    function getAttributeByTypeUpd(){
        ...
        if($type_id!==$goodsinfo['type_id']){
            ...
            dump($tmp);//整合后的三维数组信息
            $info['mark'] = 2;//商品本身拥有的属性信息
            $info[1] = $tmp;//$info变为一个四维数组
