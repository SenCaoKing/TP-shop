    ...
    function showorder(){
        ...
        if(IS_POST){
            ...
            if($new_id = $order -> add($data)){
                ...
                $url = "http://www.shop.net/zhifubao/alipayapi.php";
                //获得订单信息
                $orderinfo = D('Order') -> find($new_id);

                $arr = array(
                    'WIDout_trade_no'  => $orderinfo['order_number'],
                    'WIDsubject'       => '传智播客_牛仔裤_'.mt_rand(100,999),
                ...
                $output = curl_exec($ch);//执行一个cURL会话 抓取URL并把它传递给浏览器
